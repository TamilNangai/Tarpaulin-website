<<<<<<< HEAD
const WhyChooseUs = () => {
  return (
  
      <div className="text-xs md:text-2xl lg:text-2xl" >
        <h2 className="text-center text-2xl md:text-5xl font-[Abhaya-Libre] mb-2">Why Choose Us?</h2>

        <ul className="list-disc font-[Be-Vietnam] leading-normal">
          <li>
             Quality Materials: We use only the best materials to ensure our tarps are tough, durable.
          </li>
          <li>
              Customization: Need a specific size or feature? We offer custom tarpaulin solutions tailored to your requirement.
          </li>
          <li>
              Expertise:With years of experience in the industry, our team has the knowledge to help you find the perfect tarpaulin for any application.
          </li>
          <li>
              Customer Satisfaction:We are committed to providing exceptional customer service and ensuring our clients are completely satisfied with their purchases.
          </li>
        </ul>
      </div>
 
  );
}

export default WhyChooseUs;

=======
const WhyChooseUs = () => {
  return (
  
      <div className="text-xs md:text-2xl lg:text-2xl" >
        <h2 className="text-center text-2xl md:text-5xl font-[Abhaya-Libre] mb-2">Why Choose Us?</h2>

        <ul className="list-disc font-[Be-Vietnam] leading-normal px-5">
          <li>
             Quality Materials: We use only the best materials to ensure our tarps are tough, durable.
          </li>
          <li>
              Customization: Need a specific size or feature? We offer custom tarpaulin solutions tailored to your requirement.
          </li>
          <li>
              Expertise:With years of experience in the industry, our team has the knowledge to help you find the perfect tarpaulin for any application.
          </li>
          <li>
              Customer Satisfaction:We are committed to providing exceptional customer service and ensuring our clients are completely satisfied with their purchases.
          </li>
        </ul>
      </div>
 
  );
}

export default WhyChooseUs;
>>>>>>> e8b7b59aefcd813bd8bfb91aa9db79d6b9f4cb77
